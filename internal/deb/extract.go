	...
	for {
		...
		for targetPath, extractInfos := range targetPaths {
			...
			if tarHeader.Typeflag == tar.TypeLink {
				...
				// Use the full path for createdFiles
				...
				if exists {
					...
					// Track the created file using the correct path
					createdFiles[filepath.Join(options.TargetDir, targetPath)] = filepath.Join(options.TargetDir, targetPath)
					logf("Tracked created file: %s", createdFiles[filepath.Join(options.TargetDir, targetPath)])
				}
			} else {
				...
				createdFiles[filepath.Join(options.TargetDir, targetPath)] = filepath.Join(options.TargetDir, targetPath)
				logf("Tracked created file for potential hard link: %s", createdFiles[filepath.Join(options.TargetDir, targetPath)])
			}
		}
